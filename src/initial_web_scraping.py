''' Working on the initial web scraping of the Federal Reserve Web site '''

''' Format of the Federal reserve speaches
The base speeches are separated by year and stored on their own web page 
"/newsevents/speech/2019-speeches.htm"
"/newsevents/speech/2018-speeches.htm"
"/newsevents/speech/2006speech.htm"

BEFORE 2006 the speaches were in a different format with the same naming convention
"/newsevents/speech/2005speech.htm"  is the 
The history goes back to 1996
'''
...
def find_speaches_by_year(host, url, print_test=False):
    ...
    INPUTS:
        ...
        url         the path to the speeches for a given year
...
def create_speech_df(host, annual_htm_list):
    ...
    delete_these = df[df['link'].str.match('/pub/feds')].index
...
''' BELOW HERE SHOULD BE THE INIT FUNCTION'''
...
def main():
    ...
    #create list of all speeches contained on each web site
    date_lst, speaker_lst, title_lst, link_lst = find_speaches_by_year(host, url, print_test=True)
